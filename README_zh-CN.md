...
## 🧩 UniAI 二开差异（相对原始 CoAI）
> 说明：本节基于仓库内 `dev-notes.md`、`development-log-2026-03-03.md` 的实际开发记录整理，聚焦“我们在本项目里新增了什么、和原项目有什么具体区别”。

| 能力维度 | 原始 CoAI 基线 | UniAI 二开新增/强化 |
|---|---|---|
| 首页样式 | 以单一对话式首页为主 | 新增双首页模式：`chat`（底部输入）+ `default`（中置输入），并支持设置项 `homepage_mode` 本地持久化 |
| 预设编辑器（Mask） | 预设编辑能力较轻量 | 重构为多 Tab（模型设置/提示词/预设对话），支持 `temperature`、`top_p`、`max_tokens`、`history`、`stream` 等参数覆盖 |
| 系统提示词链路 | `description` 与 `context` 易不一致 | 实现 `description` 与 `context[0].system` 双向同步，保证编辑内容与真实发送上下文一致 |
| 预设头像 | 主要使用 Emoji | 新增图片头像上传 + 1:1 圆形裁剪（`512x512`），并让 `Emoji` 组件兼容 URL/attachment/blob/data URI |
| 会话导航 | 基础会话列表 | 新增“时间轴导航”能力：消息节点面板、收藏消息、点击跳转、`j/k` 键盘导航 |
| 文件夹管理 | 文件夹交互深度有限 | 新增完整文件夹 API + 前端树形系统，支持创建/重命名/删除/改色/拖拽移入移出/排序修复/高亮反馈/状态持久化 |
| 预设与文件夹联动 | 预设与会话归档关联较弱 | 新增 Preset-as-Folder：使用预设时自动创建或复用同名文件夹，会话升级后自动落库归档 |
| 追问建议 | 无独立异步追问链路 | 新增 Follow-up Questions：主回复后异步生成追问，通过 WebSocket 下发并挂载到对应 assistant 消息 |
| 自动标题稳定性 | 存在用户设置、路由参数、网关兼容边界问题 | 修复设置持久化、模型路由参数、Gemini 网关兼容、失败回退与“New Chat 闪回”，流程收敛为“首句截取 -> AI 标题” |
| 消息操作体验 | 操作入口与显隐策略偏传统 | 操作栏迁移到消息内容下方，支持“最新消息常显 + 历史悬停显隐 + 占位防跳动”，并区分用户/AI操作集合 |
| 工程稳定性 | 开发期存在控制台噪音与构建摩擦 | 清理 SW/Ref/KaTeX/订阅相关报错，增强 Vite 代理与 `.air.toml` 监听策略，修复 TS 配置与类型问题，并完成 DnD 迁移（`react-beautiful-dnd` -> `dnd-kit`） |

### 🔧 近期重点改造（按模块）
1. 预设链路稳定性：修复 preflight 无限转圈、条目闪烁、emoji/名称丢失、刷新后状态恢复等问题，并新增 `clientKey` 与本地持久化恢复策略。
2. 预设市场体验：所有预设卡片统一三点菜单能力；内置预设支持“复制并编辑”；自定义预设支持快速编辑与删除。
3. 文件夹 UX 补全：展开折叠动效、拖拽悬停虚线高亮、非法操作抖动反馈、颜色选择器、重命名保存/取消按钮、操作 Toast、多语言文案补齐。
4. 侧栏体验升级：历史分组（Today/Yesterday/7 days/30 days/月分组）、懒加载、折叠状态持久化、侧栏宽度拖拽、移动端边缘手势开关、快捷键增强。
5. 前端工程与联调：修复 `globals.less` 处理问题、`bootstrap` 重复初始化、代理直连缺口、TS 配置引用问题与若干页面类型冲突。
6. 后端兼容与容错：补齐用户设置落库路径、订阅分支返回、自动标题回退策略、接口返回空列表语义统一（`[]` 而非 `null`）。
